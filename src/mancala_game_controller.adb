...
package body Mancala_Game_Controller is
   ...
   function Serialize_State (State : Mancala_Game_State) return String is
      ...
      Obj.Set_Field ("winningPlayerScore", Create (State.Winning_Player_Score));
...
function Deserialize_State (JSON_Text : Ada.Strings.Unbounded.Unbounded_String) return Mancala_Game_State is
      use Ada.Strings.Unbounded;
      use GNATCOLL.JSON;
      Obj : constant JSON_Value := Read (JSON_Text);
      Board      : Mancala_Game.Board_Array (0 .. Mancala_Game.Board_Size-1):= (others => 1);-- Explicit array bounds
      Turn_Player :  Natural ;
      Is_Finished :  Boolean ;
   ...
   begin
      ...
      if Has_Field (Obj,"turnPlayer") and then Get (Obj,"turnPlayer").Kind = JSON_Int_Type then
      ...
      if Has_Field (Obj, "finished") and then Get (Obj, "finished").Kind = JSON_Boolean_Type then
      ...
      if Has_Field (Obj, "winningPlayer") and then Get (Obj, "winningPlayer").Kind = JSON_Int_Type then
         Winning_Player := Get (Obj, "winningPlayer");
      else
         Put_Line ("Error: 'winningPlayer' key is missing or not an integer.");
         raise Constraint_Error with "'winningPlayer' key is missing or invalid.";
      end if;

      -- Check if "winningPlayerScore" exists and is of the correct type
      if Has_Field (Obj, "winningPlayerScore") and then Get (Obj, "winningPlayerScore").Kind = JSON_Int_Type then
         Winning_Player_Score := Get (Obj, "winningPlayerScore");
      else
         Put_Line ("Error: 'winningPlayerScore' key is missing or not an integer.");
         raise Constraint_Error with "'winningPlayerScore' key is missing or invalid.";
      end if;

      -- Check if "message" exists and is of the correct type
      if Has_Field (Obj, "message") and then Get (Obj, "message").Kind = JSON_String_Type then
      ...
      if Has_Field (Obj, "board") and then Get (Obj, "board").Kind = JSON_Array_Type then
         ...
         for I in 0 .. Length(Arr)-1 loop
            if Get(Arr,I+1).Kind = JSON_Int_Type then
               ...
               Put_Line ("Error: 'board' key is missing or not an array of integers.");
               raise Constraint_Error with "'board' key is missing or invalid.";
            end if;
         end loop;
      end if;
   ...
   AWS.Headers.Add
      ...
      if Method = "POST" and then URI = "/v1/play" then
         ...
         begin
            if Position < 0 or else Position > 13 then
               return Result : AWS.Response.Data := AWS.Response.Build (
                  ...
                  AWS.Messages.S400) with Headers => Headers;
            end if;
         ...
         return Result : AWS.Response.Data := AWS.Response.Build (
            ...
            AWS.Messages.S404) with Headers => Headers;
      end if;
